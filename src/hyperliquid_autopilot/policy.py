...
_DEFAULT_PROJECT = "hyperliquid-autopilot"
...
def _check_max_leverage(policy: Policy, ctx: dict[str, Any], res: CheckResult) -> None:
    max_lev = policy.extra.get("max_leverage")
    if max_lev is None:
        return
    lev = ctx.get("leverage")
    if lev is None:
        return
    try:
        lev_val = Decimal(str(lev))
        lim_val = Decimal(str(max_lev))
    except (InvalidOperation, ValueError):
        return
    if lev_val > lim_val:
        res.violations.append(Violation(
            rule="max_leverage",
            message=f"leverage {lev_val}x exceeds limit {lim_val}x",
        ))


def _check_allowed_coins(policy: Policy, ctx: dict[str, Any], res: CheckResult) -> None:
    coins = policy.extra.get("allowed_coins")
    if not coins:
        return
    coin = ctx.get("coin")
    if coin is None:
        return
    allowed = [c.upper() for c in coins]
    if coin.upper() not in allowed:
        res.violations.append(Violation(
            rule="allowed_coins",
            message=f"coin {coin!r} not in allowed list {coins}",
        ))
...
def check_hyperliquid(
    ...
    """Run shared + Hyperliquid-specific checks."""
    result = check(policy, context)
    _check_max_leverage(policy, context, result)
    _check_allowed_coins(policy, context, result)
    if result.violations:
        result.allowed = False
    return result
